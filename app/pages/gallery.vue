<template>
  <NuxtLayout>
    <div>
      <HeroSection
        v-if="galleryPageDate && galleryPageDate.entry && galleryPageDate.entry.galerieInhalt"
        :title="galleryPageDate.entry.galerieInhalt[0].title"
        :subtitle="
          galleryPageDate.entry.galerieInhalt[0].untertitel
            ? galleryPageDate.entry.galerieInhalt[0].untertitel
            : ''
        "
        :background-image="galleryPageDate.entry.galerieInhalt[0].bild[0].url"
        height="400"
        :show-button="false"
      />
      ...
      <VContainer class="py-12">
        <VRow justify="center">
          <VCol cols="12" md="10">
            ...
            <div class="mb-8">
              ...
              <div class="d-flex flex-wrap gap-2 mb-6">
                <VChip
                  ...
                  :color="
                    selectedCategory === category.value ? 'primary' : undefined
                  "
                  :variant="
            ...
            <div
              v-if="!hasMore && displayedImages.length > 0"
      ...
      <VDialog v-model="lightboxOpen" fullscreen>
        <VCard>
          <VToolbar color="primary" dark>
            ...
            <span class="mx-4"
              >{{ currentIndex + 1 }} / {{ displayedImages.length }}</span
            >
            <VBtn
              icon
              @click="nextImage"
          ...
          <VCardText
            class="d-flex align-center justify-center bg-black pa-0"
...
// Categories
const categories: Ref<{ label: string; value: string | null }[]> = ref([
  { label: "Alle", value: null },
]);

const {
  data: galleryPageDate,
  error,
  pending,
  refresh,
} = await useAsyncGql({
...
const loadMoreImages = async () => {
  ...
  const newImages = await getImages(searchQuery.value, PAGE_SIZE, offset);
  ...
  if (displayedImages.value.length >= totalImages.value) {
    hasMore.value = false;
  }
...
const updateMasonryLayout = () => {
  ...
  const container = masonryContainer.value as HTMLDivElement;
  const items = container.querySelectorAll(
    ".masonry-item"
  ) as NodeListOf<HTMLElement>;
  const containerWidth = container.offsetWidth;
  const columnWidth = Math.floor(
    containerWidth / Math.floor(containerWidth / 300)
...
onMounted(async () => {
  ...
  const tags = await GqlGetGalleryTags();
...
async function getImages(search: string, limit: number, offset: number) {
  try {
    const galleryImages = await GqlGetGalleryImages({
      limit: limit,
      offset: offset,
      search: search,
    ...
    if (!galleryImages || !galleryImages.entries) {
      throw new Error("No images found");
    }

    totalImages.value = galleryImages.entryCount;

    return galleryImages.entries.map((entry) => {
      if (!entry || !Object.keys(entry).length) {
        throw new Error("No images found");
      }

      const imageData = entry as GraphQlImageData;

      return {
        id: imageData.id ?? "",
        src: imageData.bild?.[0]?.url ?? "",
        title: imageData.title ?? "",
        description: imageData.title ?? "",
        category: imageData.schlagworte?.[0]?.title ?? "all",
        // height: entry.bild?.[0]?.height,
        height: Math.floor(Math.random() * 200) + 250, // wie bisher
      };
    });
  } catch (error) {
    console.error(error);
